...
import models.services.{ AuthTokenService, UserMicroService, UserService }
...
class ActivateAccountController @Inject() (
  ...
  //      case Some(user) if !user.isRegistered =>
  //        authTokenService.create(user).map { authToken =>
  ...
  def activate(token: UUID) = silhouette.UnsecuredAction.async { implicit request =>
    authTokenService.validate(token).flatMap {
      case Some(authToken) => userService.retrieve(authToken.userID).flatMap {
        case Some(user) if user.id.toLoginInfo.providerID == CredentialsProvider.ID =>
          userService.save(user.register).map { _ =>
